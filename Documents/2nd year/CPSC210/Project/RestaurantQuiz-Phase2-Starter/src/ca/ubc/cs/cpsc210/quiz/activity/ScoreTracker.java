...
public class ScoreTracker {
    ...
    public void addScore(Score s) {
        ...
        this.minAttempts=findMinAttemptsTaken();
        this.highPointsEarned=findHighestPointsEarned();
    }
    ...
    public List<Score> getScores() {
        List<Score> newScores = new ArrayList<Score>(scores);
    ...
    private int findHighestPointsEarned() {
        int maxPoint = scores.get(0).getPointsEarned();
        for (int i=1;i<scores.size();i++){
            if (maxPoint<scores.get(i).getPointsEarned()){
                maxPoint=scores.get(i).getPointsEarned();
            }
        }
        return maxPoint;
    }
    ...
    private int findMinAttemptsTaken() {
        int minAttempt = scores.get(0).getNumAttempts();
        for (int i=1;i<scores.size();i++){
            if (minAttempt>scores.get(i).getNumAttempts()){
                minAttempt=scores.get(i).getNumAttempts();
            }
        }
        return minAttempt;
    }
